...
public class UpdatePatient extends javax.swing.JDialog {
    ...
    public UpdatePatient(java.awt.Frame parent, boolean modal, int idPatient) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        updateButton.setIcon(new javax.swing.ImageIcon(getClass().getResource("/GraphResources/UpdateButton.png"))); // NOI18N
        ...
        jPanel1.add(updateButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(350, 520, 290, 40));
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/GraphResources/UpdateSectionLogo.png"))); // NOI18N
    ...
    public void updatePatient() {
        if(verifyFullFields() && verifyEmailFormat() && verifyPhoneFormat()){
            ...
            try {
                if (this.manager.updatePatient(data, this.idPatient) > 0) {
                    JOptionPane.showMessageDialog(this, "Se ha editado exitosamente el paciente.", "Edición exitosa", JOptionPane.INFORMATION_MESSAGE);
                    this.dispose();
                }//end if sí inserta
            }//end try 
            catch (ClassNotFoundException | SQLException ex) {
                JOptionPane.showMessageDialog(this, "Hubo un error al editar el paciente.\nERROR: " + ex.getMessage());
            }//end catch exception
        }//end validaciones
    ...
    public void fillFields() {
        try {
            String[] datos = this.manager.selectPatient(this.idPatient);
            String[] partes = datos[4].split("-");
            
            this.nameField.setText(datos[0]);
            this.lastNameField.setText(datos[1]);
            
            if(datos[2].equals("M")){
                opSexGroup.setSelected(opMasculine.getModel(), true);
            }//end if
            else if(datos[2].equals("F")){
                opSexGroup.setSelected(opFemenine.getModel(), true);
            }//end else if
            else if(datos[2].equals("O")){
                opSexGroup.setSelected(opOther.getModel(), true);
            }//end else if
            
            this.maritalMenu.setSelectedItem(datos[3]);
            this.daySpinner.setValue(Integer.parseInt(partes[2]));
            this.monthMenu.setSelectedIndex(Integer.parseInt(partes[1]));
            this.yearSpinner.setValue(Integer.parseInt(partes[0]));
            this.addressField.setText(datos[5]);
            this.phoneField.setText(datos[6]);
            this.emailField.setText(datos[7]);
            this.heightField.setValue(Double.parseDouble(datos[8]));
            this.weightField.setValue(Double.parseDouble(datos[9]));
            this.tempSlider.setValue(Integer.parseInt(datos[10]));
            this.tempField.setText(datos[10]);
            this.oxiSlider.setValue(Integer.parseInt(datos[11]));
            this.oxiField.setText(datos[11]);
            this.descField.setText(datos[12]);
            
        }//end try
        catch (ClassNotFoundException | SQLException e) {
            JOptionPane.showMessageDialog(this, "Hubo un error al traer los datos del paciente.\nERROR: " + e.getMessage());
        }//end catch exception
    }//end fillFields
